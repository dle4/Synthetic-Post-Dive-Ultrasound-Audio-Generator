...
numfiles = 100; %number of synthetic Doppler files to be made
...
%     figure(1234); %visualize the single bubble segmentation process
%     hold on;
%     plot(audio);
%     xline(window_idx(:,1)-.05*Fs,'r');
%     xline(window_idx(:,2)+.05*Fs,'o');
%     hold off;
end
...
%% Load subclavian vein data into matlab and preprocess
...
for all_sim = 1:numfiles
    ...
    try
        ...
        buffered_length = desired_length_samp*1;
        ...
        y2 = movmean(y2,500); %average filter and flip the signal
...
%         findpeaks(y2,"NPeaks",npeaks, "MinPeakDistance",minpeakdist*.9); % detect troughs in the inverted cardiac signal
        window_indexes = [];
...
%% display data         
close all; figure(1234);   
k = 0.01;
subplot(311);
t = 0:1/Fs2:(length(y)-1)/Fs2; 
plot(t,y,'Color',[0 0.4470 0.7410]); hold on; title(["Cardiac only Doppler"], 'FontSize',24); ylim([-1, 1]); yl = ylim; xlabel('Time (s)','FontSize',14); ylabel('Normalized Amplitude','FontSize',14); 

window_indexes_time = window_indexes2/Fs2; 
selected_cycles_time = selected_cycles/Fs2; 
for i = 1:size(window_indexes_time,1)
    xBox = [window_indexes_time(i,1), window_indexes_time(i,1),window_indexes_time(i,2) window_indexes_time(i,2)];
    yBox = [yl(2),yl(1),yl(1),yl(2)];
    patch(xBox, yBox, 'black', 'FaceColor', 'green', 'FaceAlpha', 0.0, 'EdgeColor',[0.8500 0.3250 0.0980],'LineWidth',1.5);
end

for i = 1:size(selected_cycles_time,1)
    xBox = [selected_cycles_time(i,1), selected_cycles_time(i,1),selected_cycles_time(i,2) selected_cycles_time(i,2)];
    yBox = [yl(2),yl(1),yl(1),yl(2)];
    patch(xBox, yBox, 'black', 'FaceColor', 'green', 'FaceAlpha', 0.1, 'EdgeColor',[0.8500 0.3250 0.0980],'LineWidth',1.5);
end
...
subplot(312);
plot(t,bubble_audio,'Color',[0.8500 0.3250 0.0980]); title('Bubble only Doppler','FontSize',24); ylim([-1, 1]); yl = ylim; xlabel('Time (s)','FontSize',14); ylabel('Normalized Amplitude','FontSize',14); 
...
subplot(313); 
plot(t, combined_audio,'Color',[0 0.4470 0.7410]); hold on;  title('Combined Synthetic Doppler (KM 222)','FontSize',24);  ylim([-1, 1]); xlabel('Time (s)','FontSize',14); ylabel('Normalized Amplitude','FontSize',14); 
plot(t,bubble_audio,'Color',[0.8500 0.3250 0.0980]); 
for i = 1:size(selected_cycles_time,1)
    xBox = [selected_cycles_time(i,1), selected_cycles_time(i,1),selected_cycles_time(i,2) selected_cycles_time(i,2)];
    yBox = [yl(2),yl(1),yl(1),yl(2)];
    patch(xBox, yBox, 'black', 'FaceColor', 'green', 'FaceAlpha', 0.1, 'EdgeColor',[0.8500 0.3250 0.0980],'LineWidth',1.5);
