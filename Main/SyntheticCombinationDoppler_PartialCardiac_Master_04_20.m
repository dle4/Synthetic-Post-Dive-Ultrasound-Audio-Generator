...
% using parfor instead of for on line 152
...
%             [pks2, locs2] = findpeaks(y2,"NPeaks",npeaks, "MinPeakDistance",minpeakdist*.9); % detect troughs in the inverted cardiac signal
            ...
            for j = 1:size(pks2,1)
                locPeak = locs2(j);
                vPeak = pks2(j);
                idx_start = find(smax2(1:locPeak) < 0.5*vPeak, 1, 'last');
                idx_end = locPeak + find(smax2(locPeak:end) < 0.5*vPeak, 1, 'first');
                if idx_start > 0
                    window_idx(count,1) = idx_start;
                    if idx_end < length(curr_seg)
                        window_idx(count,2) = idx_end;
                    elseif idx_end > length(curr_seg)
                        window_idx(count,2) = length(curr_seg);
                    else
                        window_idx(count,2) = length(curr_seg);
                    end
                    count = count+1;
            ...
            win_idx = [window_idx(:,1)'; window_idx(:,2)'];
            win_idx2 = win_idx(:);
            win_idx2 = sort(win_idx2);
            win_idx3 = [1; win_idx2; length(y)];
            win_idx4 = [win_idx3(1:2:end,1) win_idx3(2:2:end,1)];

            if win_idx4(1,2) < 9*avg_bbl_lngth
                win_idx4(1,:) = [];
            end
            if win_idx4(end,1) > length(y)
                win_idx4(end,:) = [];
            end

            if win_idx4(end,2)-win_idx4(end,1) < 4*avg_bbl_lngth
                win_idx4(end,:) = [];
            end

            check = win_idx4(:,2)-win_idx4(:,1);
            win_idx4(check<4*avg_bbl_lngth,:) = [];
            window_indexes2 = win_idx4; %carryover from other script

            avg_bbl_lgnth_sec = avg_bbl_lngth/Fs2;
            audio_length = length(rand_sample_aug);

            %double check that the size of the indexes is greater than
            %number of peaks
            if size(window_indexes2,1) > round(npeaks*.8)
                km_choice = sf{f};
